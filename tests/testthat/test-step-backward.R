...
test_that("backward selection fails when model inherits other than 'lm'", {
    ...
    m <- glm(x ~ y)
    expect_error(ols_step_backward(m), 'Please specify a OLS regression model.')
})

test_that("backward selection fails when penter is not between 0 and 1", {
	model <- lm(y ~ x1 + x2 + x3 + x4, data = cement)
  expect_error(ols_step_backward(model, prem = -0.5), 'p value for removing variables from the model must be between 0 and 1.')
  expect_error(ols_step_backward(model, prem = 1.5), 'p value for removing variables from the model must be between 0 and 1.')
})

test_that("backward selection fails when model contains less than 2 predictors", {
	model <- lm(y ~ x1, data = cement)
  expect_error(ols_step_backward(model), 'Please specify a model with at least 2 predictors.')
})
