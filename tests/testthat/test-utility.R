...
test_that('output from cpdata matches the expected result', {
    ...
    expect_equivalent(actual[[1]],
        c(6, 6, 4, 6, 8, 6, 8, 4, 4, 6, 6, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4, 8, 8,
          8, 8, 4, 4, 4, 8, 6, 8, 4))
    expect_equivalent(actual[[2]], c(-2.9203498, -2.0330346, -2.3992081,
...
test_that('output from cdplot matches the expected output', {
    model <- lm(mpg ~ disp + hp + wt + drat, data = mtcars)
    k <- cdplot(model)
    expect_equal(round(k$maxx, 3), 0.494)
