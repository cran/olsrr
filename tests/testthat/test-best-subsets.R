...
test_that("all_subsets fails when model inherits other than 'lm'", {
  ...
  m <- glm(x ~ y)
  expect_error(ols_step_best_subset(m), "Please specify a OLS linear regression model.")
})

test_that("all_subsets returns an error when number of predictors < 2", {
  model <- lm(y ~ x1, data = cement)
  expect_error(ols_step_best_subset(model), "Please specify a model with at least 2 predictors.")
})
