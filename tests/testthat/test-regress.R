...
test_that("regress fails when input for data is missing", {
  expect_error(
    ols_regress(mpg ~ disp + hp + wt + drat + qsec),
    "data missing"
  )
})
...
test_that("regress fails when input for data is other than data frame", {
  ...
  expect_error(
    ols_regress(mtcars$mpg ~ mtcars$hp + mtcars$disp, data = mat_mt),
...
test_that("regress fails when input for conf.level is non-numeric", {
  cl <- "1"
  expect_error(ols_regress(
    mpg ~ disp + hp + wt + drat + qsec, data = mtcars,
    conf.level = cl
...
test_that("regress fails when input for title is not a string/character", {
  ...
  expect_error(ols_regress(
    mpg ~ disp + hp + wt + drat + qsec, data = mtcars,
    title = title
  ), "0.95 is not a string, Please specify a string as title.")

  title <- TRUE
  expect_error(ols_regress(
    mpg ~ disp + hp + wt + drat + qsec, data = mtcars,
    title = title
  ), "TRUE is not a string, Please specify a string as title.")
})
